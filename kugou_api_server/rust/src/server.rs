...
/// Device identity cookies must be stable across process restarts, otherwise
/// every app launch (which restarts this local server) presents a brand-new
/// device upstream and the account accumulates device entries + trips risk
/// control. Delegate to the persisted DeviceConfig (device_info.json) instead
/// of generating a fresh random value each process start.
fn session_guid() -> String {
    DeviceConfig::instance().get_guid()
}

fn session_mid() -> String {
    DeviceConfig::instance().get_mid()
}

fn session_dev() -> String {
    DeviceConfig::instance().get_server_dev().to_uppercase()
}
...
fn handle_request(mut request: Request, _data_dir: &str) -> Result<(), String> {
    ...
    // 持久化 dfid 兜底：新建/删除歌单等写操作依赖 dfid cookie；若客户端
    // 丢失 dfid 或写请求先于 register 到达，dfid 会降级为 "-" 被上游风控拒。
    // 已知 dfid 时随每个响应回填，与其它平台 cookie 一致，确保写请求稳定持
    // 有真实 dfid（与 device_info.json 相同，不会因冷启动变化）。
    let persisted_dfid = DeviceConfig::instance().get_dfid();
    if !persisted_dfid.is_empty() && persisted_dfid != "-" {
        ensure_cookie(
            &mut cookies,
            "dfid",
            &persisted_dfid,
